...
public class BuildManager : MonoBehaviour
{
    ...
    private void OnDestroy()
    {
        ...
        if (InputManager.Instance != null) { InputManager.Instance.OnCellClicked -= HandleCellClicked; InputManager.Instance.OnAreaSelected -= HandleAreaSelected; InputManager.Instance.OnAreaDragging -= HandleAreaDragging; }
    }
    ...
    void PrevievArea(Vector3 startPos, Vector3 endPos)
    {
        ...
        for (int x = bounds.startX; x <= bounds.endX; x++)
        {
            for (int y = bounds.startY; y <= bounds.endY; y++)
            {
                ...
                if(CurrentMode == GameMode.CancelOrders && JobManager.Instance.jobQueues[JobType.Build].GetJobAt(currentCell) is BuildJob buildJob)
                {
                ...
                else if(CurrentMode == GameMode.DeconstructBuilding && JobManager.Instance.buildings.ContainsKey(currentCell))
                {
                    GameReference.Instance.HighlightTilemap.SetTile(currentCell, GameReference.Instance.SelectionBoxHighlightTile);
                    previewCells.Add(currentCell);
                }
                else if(CurrentMode == GameMode.CancelOrders && JobManager.Instance.jobQueues[JobType.Deconstruct].GetJobAt(currentCell) is DeconstructJob deconstructJob)
                {
                    GameReference.Instance.HighlightTilemap.SetTile(currentCell, GameReference.Instance.SelectionBoxHighlightTile);
                    previewCells.Add(currentCell);
                }
    ...
    void ClearPreviev()
    {
        foreach (Vector3Int cell in previewCells)
        {
            ...
            else if (CurrentMode == GameMode.CancelOrders && JobManager.Instance.jobQueues[JobType.Build].HasJobAt(cell))
            {
                GameReference.Instance.HighlightTilemap.SetTile(cell, null);
            }
            else if(CurrentMode == GameMode.CancelOrders && JobManager.Instance.jobQueues[JobType.Deconstruct].HasJobAt(cell))
            {
