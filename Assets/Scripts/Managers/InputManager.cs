...
public class InputManager : MonoBehaviour
{
    ...
    void MouseInput()
    {
        if (Input.GetMouseButtonDown(0) && !EventSystem.current.IsPointerOverGameObject() && (currentMode == GameMode.Digging || currentMode == GameMode.CancelOrders ||  currentMode == GameMode.DeconstructBuilding))
        {
        ...
        else if (Input.GetMouseButton(0) && selectionBox.gameObject.activeSelf && (currentMode == GameMode.Digging || currentMode == GameMode.CancelOrders || currentMode == GameMode.DeconstructBuilding))
        {
            Vector3 currentStartScreenPos = mainCamera.WorldToScreenPoint(startMousePos);
            Vector3 currentMouseScreenPos = Input.mousePosition;
            selectionBox.position = (currentStartScreenPos + currentMouseScreenPos) / 2;

            float width = Mathf.Abs(currentMouseScreenPos.x - currentStartScreenPos.x) / mainCanvas.scaleFactor;
            float height = Mathf.Abs(currentMouseScreenPos.y - currentStartScreenPos.y) / mainCanvas.scaleFactor;
        ...
        else if (Input.GetMouseButtonUp(0) && selectionBox.gameObject.activeSelf && (currentMode == GameMode.Digging || currentMode == GameMode.CancelOrders || currentMode == GameMode.DeconstructBuilding))
        {
        ...
        if(Input.GetMouseButtonUp(0) && !EventSystem.current.IsPointerOverGameObject()) OnCellClicked?.Invoke(GetMouseCellPosition());
